...
### Ensemble Function
def test(opt, models, data, split, device):
    model.eval()
    ...
    for batch_idx, (x_patname, x_omic, censor, survtime, grade) in enumerate(test_loader):
        ...
        for model in models:
            ...
            with torch.no_grad():
                ...
                ensemble_predictions.append(pred.unsqueeze(0))  # Add batch dimension for aggregation
                ensemble_features.append(features.unsqueeze(0))
        # Aggregate predictions from all models
        final_prediction = aggregate_predictions(ensemble_predictions)
        # Average features from all models
        average_features = torch.mean(torch.cat(ensemble_features, dim=0), dim=0)

        loss_cox = CoxLoss(survtime, censor, final_prediction, device) if opt.task == "surv" else 0
        loss_reg = definereg(opt, model)
        loss_nll = F.nll_loss(final_prediction, grade) if opt.task == "grad" else 0
        ...
        if opt.task == "surv":
            ...
            features_all.append(average_features.detach().cpu().numpy().reshape(-1))
            risk_pred_all = np.concatenate((risk_pred_all, final_prediction.detach().cpu().numpy().reshape(-1)))
            ...
            # Assuming class_labels are the true labels for the classification task
            # test_pred_all = np.concatenate((test_pred_all, pred.detach().cpu().numpy()), axis=0)
            preds = np.argmax(final_prediction.detach().cpu().numpy(), axis=1)
...
for fold in range(1, 11):
    if opt.cancer is None:
        load_path = os.path.join(opt.checkpoints_dir, opt.exp_name, opt.model_name, opt.disease, f'Checkpoint_{opt.disease}_{opt.model_name}_fold{fold}_for_test.pt')
        ...
        load_path = os.path.join(opt.checkpoints_dir, opt.exp_name, opt.model_name, opt.disease, opt.cancer, 'Finetuned_Checkpoint_%s_%s_fold%d_for_test_splits1_May6.pt' % (opt.disease, opt.model_name, fold))
    elif opt.cancer == 'CPTAC-LUSC':
        print("############### Cancer Type is CPTAC-LUSC ####################")
        # load_path = os.path.join(opt.checkpoints_dir, opt.exp_name, opt.model_name, opt.disease, 'Checkpoint_%s_%s_fold%d_for_test.pt' % (opt.disease, opt.model_name, fold))
        load_path = os.path.join(opt.checkpoints_dir, opt.exp_name, opt.model_name, opt.disease, opt.cancer, 'Finetuned_Checkpoint_%s_%s_fold%d_for_test_May6.pt' % (opt.disease, opt.model_name, fold))
    else:
        load_path = os.path.join(opt.checkpoints_dir, opt.exp_name, opt.model_name, opt.disease, opt.cancer, f'Checkpoint_{opt.disease}_{opt.model_name}_fold{fold}_for_test.pt')
        load_path = os.path.join(opt.checkpoints_dir, opt.exp_name, opt.model_name, opt.disease, opt.cancer, 'Finetuned_Checkpoint_%s_%s_fold%d_for_test_May6.pt' % (opt.disease, opt.model_name, fold))
    fold_checkpoints.append(load_path)

for load_path in fold_checkpoints:
    ...
    models.append(model)
# 'models' now contains all the loaded models

# Define a function to aggregate predictions (example for classification with voting)
def aggregate_predictions(predictions):
    # For classification, you can use voting (most common class among models)
    # For regression, you can average the predictions
    predictions = torch.stack(predictions)  # Stack predictions to form a tensor [num_models, num_samples, ...]
    # majority_vote, _ = predictions.mode(dim=0)  # For Classification task, compute mode along the model dimension
    average_prediction = torch.mean(predictions, dim=0) # Compute average along the model dimension
    return average_prediction #, majority_vote

if opt.cancer is None:
    # data_cv_path_template = os.path.join(opt.dataroot, opt.disease, 'test', '{}_test_10cv_fold_{}.pkl'.format(opt.model_name, '{}'))
...
for k, data in data_cv_splits.items():
    ...
    print("********************************************")
    loss_test, cindex_test, pvalue_test, surv_acc_test, pred_test, pat_features, class_acc_test = test(opt, models, data, 'test', device)
    ...
    if opt.cancer is None:
        # pickle.dump(pred_test, open(os.path.join(opt.checkpoints_dir, opt.exp_name, opt.model_name, opt.disease, 'Inference_Ensemble_predictions_%s_%s_fold%d.pkl' % (opt.disease, opt.model_name, k)), 'wb')) # fold-wise
        pickle.dump(pred_test, open(os.path.join(opt.checkpoints_dir, opt.exp_name, opt.model_name, opt.disease, 'Inference_Ensemble_predictions_%s_%s.pkl' % (opt.disease, opt.model_name)), 'wb'))
    else:
        pickle.dump(pred_test, open(os.path.join(opt.checkpoints_dir, opt.exp_name, opt.model_name, opt.disease, opt.cancer, 'Inference_Ensemble_predictions_%s_fold%d.pkl' % (opt.cancer, fold)), 'wb'))
...
if opt.task == 'class':
    ...
    if opt.cancer is None:
        pickle.dump(results_test, open(os.path.join(opt.checkpoints_dir, opt.exp_name, opt.model_name, opt.disease, 'Inference_Ensemble_CIndex_%s_%s.pkl' % (opt.disease, opt.model_name)), 'wb'))
    else:
        pickle.dump(results_test, open(os.path.join(opt.checkpoints_dir, opt.exp_name, opt.model_name, opt.disease, opt.cancer, 'Inference_Ensemble_CIndex_%s.pkl' % (opt.cancer)), 'wb'))
